...
io.on('connection', (socket) => {
  ...
  socket.on('message', (data) => {
    ...
    db.run('INSERT INTO messages (username, content, is_from_sam) VALUES (?, ?, ?)', 
      ...
      notifyDiscord(socket.username, sanitizedContent);
    });
  });
...
async function notifyDiscord(username, message) {
  try {
    const channel = await discordClient.channels.fetch(process.env.DISCORD_CHANNEL_ID);
    if (!channel) return;
    
    const threadName = `User_${username}`;
    ...
    if (!thread) {
      thread = await channel.threads.create({
        ...
        reason: `New user conversation: ${username}`
      });
    }
    
    await thread.send(`**${username}**: ${message}`);
    console.log(`[DISCORD_SENT] Message routed to thread: ${threadName}`);
  } catch (error) {
    console.error('[DISCORD_ERROR] Failed to send message:', error);
  }
}

discordClient.on('messageCreate', async (message) => {
  if (message.author.bot) return;
  if (message.channel.isThread() && message.channel.name.startsWith('User_')) {
    const username = message.channel.name.replace('User_', '');
    const socketId = userSockets.get(username);
    
    if (socketId && message.author.id === process.env.DISCORD_USER_ID) {
      const content = dompurify.sanitize(message.content);
      
      if (content === 'did you know you can paint over the site and shit') {
        userPaintPermissions.set(username, true);
        io.to(socketId).emit('paint_permission_granted');
        console.log(`[PAINT_UNLOCK] Paint permission granted to ${username}`);
        return;
      }
      
      db.run('INSERT INTO messages (username, content, is_from_sam) VALUES (?, ?, ?)', 
        [username, content, 1], function(err) {
        if (!err) {
          const response = {
            id: this.lastID,
            username: username,
          ...
          io.to(socketId).emit('sam_response', response);
          console.log(`[SAM_REPLY] Response sent to ${username}`);
        }
      });
    }
  }
...
async function startServer() {
  ...
  try {
    ...
    console.log('[DISCORD_READY] Bot connected');
  } catch (error) {
    console.error('[DISCORD_ERROR] Failed to connect:', error);
  }
